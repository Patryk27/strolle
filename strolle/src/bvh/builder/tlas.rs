...
use crate::primitives::TlasPrimitives;
use crate::{Axis, BoundingBox, Params, PrimitiveId, PrimitivesRef};

const BINS: usize = 12;

pub fn run<P>(primitives: &mut TlasPrimitives<P>, nodes: &mut BvhNodes)
where
    P: Params,
{
    ...
    thread::scope(|s| {
        s.spawn(|| {
            ...
            while let Some(node) = stack.pop_front() {
                match balance(primitives, nodes, node) {
...
#[inline(always)]
fn balance<P>(
    primitives: &mut TlasPrimitives<P>,
    nodes: &mut BvhNodes,
    node_ref: BvhNodeRef,
    ...
    P: Params,
{
    ...
    let left_primitives_ref =
        PrimitivesRef::range(primitives_ref.start(), pivot);
    let right_primitives_ref =
        PrimitivesRef::range(pivot, primitives_ref.end());
