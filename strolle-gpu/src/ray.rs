...
use glam::{vec3, Affine3A, Mat3A, Vec3, Vec4, Vec4Swizzles};
...
impl Ray {
    ...
    fn traverse(
        ...
        let mut bvh_ptr = bvh.get(0).x.to_bits();
        ...
        let mut prev_origin = Vec3::ZERO;
        let mut prev_direction = Vec3::ZERO;
        ...
        loop {
            ...
            match d0.w.to_bits() {
                ...
                0 => {
                    ...
                    let mut near_distance =
                        self.intersect_box(d0.xyz(), d1.xyz());

                    let mut far_distance =
                        self.intersect_box(d2.xyz(), d3.xyz());
                    ...
                    // If the nearest child is closer than our current best
                    // shot, let's check that child first; use stack to save the
                    // other node for later.
                    //
                    // The reasoning here goes that the closer child is more
                    // likely to contain a triangle we can hit; but if we don't
                    // hit that triangle (kind of a "cache miss" kind of thing),
                    // we still have to check the other node.
                ...
                // Got a leaf node (that contains a triangle)
                1 => {
                    used_memory += mem::size_of::<Triangle>();

                    let flags = d0.x.to_bits();

                    // Whether there are any more triangles directly following
                    // this triangle.
                    //
                    // This corresponds to a single BVH leaf node containing
                    // multiple triangles.
                    let got_more_entries = flags & 1 == 1;

                    // Whether the triangle we're looking at supports alpha
                    // blending.
                    //
                    // If this is turned on, we have to load the triangle's
                    // material and compute albedo to make sure that the part of
                    // triangle we hit is actually opaque at that particular hit
                    // point.
                    ...
                    let mut found_hit =
                        triangles.get(triangle_id).hit(self, hit);
                ...
                // Got a leaf node (that contains a nested BVH for some mesh)
                _ => {
                    used_memory += 3 * mem::size_of::<Vec4>();
                    ...
                    let xform = Affine3A {
                        ...
                        translation: vec3(d1.w, d2.w, d3.w).into(),
                    };

                    prev_origin = self.origin;
                    prev_direction = self.direction;

                    self.origin = xform.transform_point3(self.origin);
                    self.direction = xform.transform_vector3(self.direction);
                    self.inv_direction = 1.0 / self.direction;

                    if got_more_entries {
                        unsafe {
                            *stack.index_unchecked_mut(stack_ptr) = bvh_ptr + 4;
                            stack_ptr += 1;
                        }
                    }

                    bvh_ptr = d0.y.to_bits();

                    unsafe {
                        *stack.index_unchecked_mut(stack_ptr) = 0xffffffff;
                        stack_ptr += 1;
                    }
            ...
            // tested all nodes and we can bail out.
